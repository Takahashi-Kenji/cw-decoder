...
#
# 区別できないことの代償は実際に払っていた。辞書補正は ``?`` を「どの文字にも
# 安く化けるワイルドカード」として扱うため、本物の ``?`` まで飲み込んでしまう。
# それを防ぐために「末尾の ``?`` を外して語彙語になれば本物」という**推測**の
# 歯止めを欧文・和文の両方に入れていた (無いと ``QSL?`` → ``QSL`` と削って
# 0.42pt 悪化した)。記号を分ければ、この推測は要らなくなる。
#
# ``_`` は欧文表・和文表・送信専用表のいずれにも存在しないので衝突しない
# (2026-08-14 に確認。運用者の提案)。
FALLBACK_CHAR: Literal["_"] = "_"
